    ...
    sub new {
        ...
        my %dispatch_table = (
            ...
            'metrics_dependabot' => sub {
                my %metrics_message =
                  ( %flow_message, metric_scope => 'dependabot' );
                return Sentra::Component::DependabotMetrics->new(
                    \%metrics_message );
            },
            'metrics_secret' => sub {
                my %metrics_message =
                  ( %flow_message, metric_scope => 'secret' );
                return Sentra::Component::DependabotMetrics->new(
                    \%metrics_message );
            },
            'metrics_code' => sub {
            ...
            'dependency' =>
              sub { Sentra::Component::SearchFiles->new( \%flow_message ) },
            'maintained' =>
              sub { Sentra::Component::Maintained->new( \%flow_message ) },
            'security_tools' =>
              sub { Sentra::Component::SecurityTools->new( \%flow_message ) },
        );

        for my $option (
            qw(dependency maintained metrics metrics_dependabot metrics_secret metrics_code security_tools)
          )
        {
        ...
        if ( $webhook && $message ) {
            ...
            my $send_result =
              Sentra::Component::SlackWebhook->new( \%slack_message );
